...
if (isset($_POST['checkbox_pv'])) {
$checkbox_pv = $_POST['checkbox_pv'];
//echo $checkbox_pv;
  if ($checkbox_pv == 'on') {
    echo $checkbox_pv;
      $Status_tsinjo = 'actif';
      $action = 'remove';
      $query_c = "INSERT INTO depense(depense_tsinjo, motif,activity_no,user_depense) VALUES (?, ?, ?,?)";

      $q = $bdd->prepare($query_c);

      $q->execute(array($depense, $motif,$activity_no,$username));
      $q->closeCursor();
      $depense = 0;
}
}
...
if ($depense != 0) {
      $query_c = "INSERT INTO depense(montant, motif,activity_no,user_depense) VALUES ( ?, ?, ?,?)";
...
if ($depense_aparafa != 0) {
      $query_c = "INSERT INTO depense(depense_aparafa, motif,activity_no,user_depense) VALUES (?, ?, ?,?)";

      $q = $bdd->prepare($query_c);

      $q->execute(array($depense_aparafa, $motif,$activity_no,$username));  
