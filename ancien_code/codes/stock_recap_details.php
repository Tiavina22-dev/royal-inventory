...
<a href="stock_recap_details.php"><button type="submit" class="btn btn-info">RETOURS</button></a>
<a class="btn btn-brown"
href="stockpd1.php?nom_client_fournisseur=<?php echo urlencode($nom_client_fournisseur); ?>&description_date=<?php echo urlencode($description_date); ?>&no_activite=<?php echo urlencode($no_activite); ?>">
Facture
</a>
...
<div>
    ...
    <!-- Nouveau select Point de Vente -->
    <div class="form-group">
        <label><b>Point de vente</b></label>
        <select class="form-control" name="nom_client_fournisseur">
            <?php
                // Exemple: récuperation des points de vente depuis la table 'clients_fournisseurs'
                $pv_query = $bdd->query("SELECT DISTINCT nom_client_fournisseur FROM mvt ORDER BY nom_client_fournisseur");
                while($pv = $pv_query->fetch()) {
                    $selected = ($pv['nom_client_fournisseur'] == $nom_client_fournisseur) ? "selected" : "";
                    echo "<option value='".$pv['nom_client_fournisseur']."' $selected>".$pv['nom_client_fournisseur']."</option>";
                }
                $pv_query->closeCursor();
            ?>
        </select>
    </div>
...
<!------------------------------------------------->
 ...
 <!--------<td class="text-center"><a href="#View" data-toggle="modal" data-target="#<?php echo "img".$donnees['id_x']; ?>"><img src="<?php echo $image_path_x; ?>" height="50" width="50" background alt="Edit" /></a></td>------->
        ...
        <td class="text-right"><?php echo $MT; ?></td>
...
<tr style="background: #607d8b" class="text-warning">
		        <th colspan="4" class="text-center">#####TOTAL STOCK#####</th>
		        <th></th>
		        <th  class="text-right"><b><?php echo number_format($Total,0, "", " "); ?></b></th>
		        <th></th>
		        <th></th>
		        <th></th>
		        <th></th>
