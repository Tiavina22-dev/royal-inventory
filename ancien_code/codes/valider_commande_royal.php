...
while ($donnees = $query_get_current_commande -> fetch())
{
	...
	$query_c = "INSERT INTO mvt(type_de_mvt, id_x, qt, prix_unitaire,prix_aparafa,nom_client_fournisseur,description_date,numero_commande_stock,ref_commande_stock,note,user_mvt,Date_du_Journal_mvt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$status ="NON_RESOLU";
	...
	$q->execute(array($numero_commande,$no,$versement,$difference_aparafa,$resolution,$mihoatra,$history,$note_general,$point,$path,$status,$date1_mysql,'No_Change'));	
