...
//Query to liste negatif product
   $query_stock_negatif = "SELECT nom_client_fournisseur, produit.id_x as id_x, nom_x, SUM(qt) as sm, reference_x FROM mvt NATURAL JOIN produit WHERE nom_client_fournisseur = ? AND (description_date LIKE '%balance_zero%') GROUP BY id_x;";
    ...
    $nb_line=$q->rowCount ();   
?>
<h4 class="text-center"><?php echo $nom_client_fournisseur;?> || STOCK BALANCED ZERO (<?php echo $nb_line;?>)</h4>
                ...
                <div class="modal-content">
                    ...
                    <form role="form" action="balance_negatif.php" method="post">
                        ...
                        { 
                            ...
                            if ($donnees1['description_date']=='balance_zero') {
                        ?>
                        <li class="list-group-item"><?php echo $donnees1['description_date'].' ('.$donnees1['date_time'].')'; ?> | <a href="delete_mvt_stock_bz.php?id_mvt=<?php echo $donnees1['id_mvt']; ?>&nom_client_fournisseur=<?php echo $donnees1['nom_client_fournisseur']; ?>" onclick="confirmationDelete('Do you want to DELETE?');return false; post ;">Annuler?</a><span class="w3-badge w3-right w3-margin-right w3-red"><?php echo $donnees1['qt']; ?></span></li>
                        <?php
                            } else {
                        ?>
