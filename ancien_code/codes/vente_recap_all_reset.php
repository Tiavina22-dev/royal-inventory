...
//---------SET status To RESOLU-------------------
 //Query to liste All Journal de vente
    $query_journal_vente = "SELECT *,recap_vente.no_activite as no_activite FROM recap_vente INNER JOIN mvt ON recap_vente.no_activite = mvt.numero_commande_stock WHERE type_de_mvt = 'vente' AND nom_client_fournisseur = ? AND status ='NON_RESOLU' GROUP BY mvt.numero_commande_stock ORDER BY id DESC;";
...
header("location: vente_recap_bejofo.php");
