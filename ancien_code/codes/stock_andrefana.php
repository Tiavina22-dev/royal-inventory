...
<head>
<meta charset="utf-8">
<title>Ajouter Stock Andrefana</title>
...
if (isset($_COOKIE['msg_validation'])) 
{
   ...
   $vers_controle = 'Afficher stock ANDREFANA?';
   //----PLAY SOUND-------------------------
	?>
...
<!-- actual form -->
<form role="form" action="new_stock_andrefana.php" method="post">
...
##########################################################
$client_name = "";
		$query_client_name = $bdd->prepare("SELECT * FROM stock_prep_calc WHERE description_date LIKE '%Ajout%' AND user_stock_prep = ?");
...
<br>
<br>
<form role="form" action="simple_search_andrefana.php" method="POST">
<div class="text-center">
		...
		<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="<?php echo 'no'.$donnees['id_x']; ?>" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<!-- actual form -->
					<form role="form" action="insert_stock_prep_andrefana.php" method="post">
						<div class="form-group">
...
# QUERY GET DISTINCT USER
		$query_user = $bdd->prepare("SELECT user_stock_prep,nom_du_client,numero_stock_prep,description_date FROM stock_prep_calc WHERE description_date LIKE '%Ajout%' GROUP BY user_stock_prep");
...
?>
	...
	<div>
        ...
        <th class="text-right">QT</th>
        <?php if ($client_name == 'Ambato_Tantely') {echo "<th class='text-right'>PU TANTELY</th>";} ;?>
        <?php if ($client_name == 'Amparafa') {echo "<th class='text-right'>PU AMPARAFA</th>";} ;?>
        <?php if (($client_name != 'Ambato_Tantely') AND ($client_name != 'Amparafa')) {echo "<th class='text-right'>PU GENERAL</th>";} ;?>
        <th></th>
        <th class="text-right">MONTANT</th>
        <th>NOTE</th>
        <?php
...
?>
		...
		<td><a class="center" href="delete_stock_prep_andrefana.php?id_stock_prep=<?php echo $donnees['id_stock_prep']; ?>" onclick="confirmationDelete('Do you want to DELETE this line?');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit" /></a></td>
		<?php
		}//Close else 
		?>
<!-------FORM DE MODIFIER-------->
		...
		<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="<?php echo "no".$donnees['id_stock_prep']; ?>" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
					<!-- actual form -->
					<form role="form" action="modifier_stock_andrefana.php" method="post">
						<div class="form-group">
						<label>Quantite (Ex: Atsasany : 0.5 ; Fefany : 0.25)</label>
...
?>
<a class="center" href="anuler_stock_andrefana.php" onclick="confirmationDelete('Anuler cette action?');return false; post ;"><button type="button" class="btn btn-danger">Anuler</button></a>
<?php
}// close ELSE
 ?>
<br>
