...
$query_current_commande ="SELECT id_commande,numero_commande,nom_du_client,nom_x,commande.id_x as id_x,description_date,reference_x,note_commande,qt,commande.prix_de_vente as prix_de_vente,commande.prix_client as prix_client,(commande.prix_de_vente)*qt as sous_total,(commande.prix_client)*qt as sous_total_client FROM commande INNER JOIN produit ON commande.id_x = produit.id_x WHERE commande.user = ?;";
...
while ($donnees = $query_get_current_commande -> fetch())
{
	...
	$history = $history."=> ".$no." # ".$donnees['nom_x']." | ".$note." # qt: ".$donnees['qt']." # PU_ROY: ".$prix_unitaire." # PU_CLI: ".$prix_client." # MT_ROY: ".$donnees['sous_total']." # ".strtoupper($nom_du_client)." : ".($donnees['sous_total_client'])." # BC: ".($prix_client-$prix_unitaire)*$donnees['qt']." #"."\r";
	$somme_client = $donnees['sous_total_client']+$somme_client;
	...
	$query_c = "INSERT INTO mvt(type_de_mvt, id_x, qt, prix_unitaire,prix_client,nom_client_fournisseur,description_date,numero_commande_stock,ref_commande_stock,note,user_mvt,Date_du_Journal_mvt) VALUES (?, ?, ?, ?, ?, ?,?, ?, ?, ?, ?, ?)";

	$q = $bdd->prepare($query_c);

	$q->execute(array($type_de_mvt, $id_x, $qt, $prix_unitaire,$prix_client,$nom_du_client,$description_date,$numero_commande,$ref_commande,$note,$username,$date1_mysql));	

	$q->closeCursor();
}
//Add TOTAL AND VERSEMENT on history
$difference_client = 0;
if ($nom_du_client == 'Amparafa' OR $nom_du_client == 'Soalazaina' OR $nom_du_client == 'Bejofo') {
	$difference_client = $somme_client-$somme;
}
/*
...
$history = $history."\r"."----------------------------------------------------------------"."\r"."----------TL VENTE:".$somme." Ar----------"."\r"."----------------------------------------------------------------"."\r"."-----------------SPECIAL ".strtoupper($nom_du_client)."------------------"."\r"." IVAROTANY TL:".$somme_client." Ar "." | BENEFICE:".$difference_client." Ar | NALAINY:".$somme_depense_aparafa."\r"."------------------------DEPENSE---------------------------"."\r"."DEPENSE ROYAL:".$somme_depense." Ar | VOLA NALAIN NY ".strtoupper($nom_du_client).":".$somme_depense_aparafa." Ar | TL DEPENSE:".$all_depense." Ar"."\r"."-----------------------------------------------------------------"."\r"."--------NET POUR ROYAL:".$versement." Ar-------"."\r"."-----------------------------------------------------------------";
//--------QUERY TO INSERT ON RECAP_VENT-------
echo $date1_mysql;
echo "<br>";
$status ="NON_RESOLU";
	$query_c = "INSERT INTO recap_vente(no_activite, nb_ligne, Montant,difference_aparafa,resolution,mihoatra,history,note_general,c_point,directory,status,Date_du_Journal,responsable) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

	$q = $bdd->prepare($query_c);

	$q->execute(array($numero_commande,$no,$versement,$difference_client,$resolution,$mihoatra,$history,$note_general,$point,$path,$status,$date1_mysql,'No_Change'));	
