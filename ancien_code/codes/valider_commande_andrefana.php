...
$query_current_commande ="SELECT id_commande,numero_commande,nom_du_client,nom_x,vente_calc.id_x as id_x,description_date,reference_x,note_commande,qt,vente_calc.prix_de_vente as prix_de_vente,vente_calc.prix_aparafa as prix_aparafa, (vente_calc.prix_de_vente)*qt as sous_total,(vente_calc.prix_aparafa)*qt as sous_total_aparafa FROM vente_calc INNER JOIN produit ON vente_calc.id_x = produit.id_x WHERE vente_calc.user = ?;";
$query_get_current_commande = $bdd->prepare($query_current_commande);

 $query_get_current_commande->execute(array($username));
 $nb = $query_get_current_commande->rowCount ();
 echo $nb;
//GET NUMERO ACTIVITY
...
while ($donnees = $query_get_current_commande -> fetch())
{
	...
	$query_c = "INSERT INTO mvt_calc(type_de_mvt, id_x, qt, prix_unitaire,prix_aparafa,nom_client_fournisseur,description_date,numero_commande_stock,ref_commande_stock,note,user_mvt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
$status ="NON_RESOLU";
	$query_c = "INSERT INTO recap_vente_calc(no_activite, nb_ligne, Montant,difference_aparafa,resolution,mihoatra,history,note_general,c_point,directory,status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
//-----------DELETE COMMANDE------------
include('anuler_commande_2.php');
include('valider_stock_andrefana_retours.php');
include('valider_stock_andrefana.php');
