...
if (isset($_POST['pu_soalazaina'])) {
	$pu_soalazaina = $_POST['pu_soalazaina']+0;
}
...
//VERIFY CHANGEMENT DE PRIX
	...
	//Save note change
	if ($current_note_x != $note_x)
		{ $change_prix = "YES";
		//history
		$before_change = ' | '.$current_note_x.'</br>';
		$after_change = " | Note : <span class ='text-danger'><b>".$note_x.'</b></span></br>';
		}

	if ($difference_prix_fournisseur != 0)
		{ $change_prix = "YES";
		//history
		$before_change = $before_change.' | PU Fournisseur : '.($data['prix_fournisseur']+0).'</br>';
		$after_change = $after_change." | PU Fournisseur : <span class ='text-danger'><b>".$prix_fournisseur.'</b></span></br>';
		}
	...
	if ($difference_prix_global != 0) {
		...
		$change_prix = "YES";
		//history
		$before_change = $before_change.' | PU Global : '.$prix_avant_global.'</br>';
		$after_change = $after_change." | PU Global : <span class ='text-danger'><b>".$prix_de_vente.'</b></span></br>';
	...
	if ($difference_prix_tantely != 0) {
		...
		$change_prix = "YES";
		//history
		$before_change = $before_change.' | PU Tantely : '.$prix_avant_tentely.'</br>';
		$after_change = $after_change." | PU Tantely : <span class ='text-danger'><b>".$pu_tantely.'</b></span></br>';
		//To handle empty note field
		if (strlen($note_x) == 0) {
			$note_x = "latest";
		}
	...
	//-----Calcule difference de prix Soalazaina------
	$prix_avant_soalazaina = $data['pu_soalazaina']+0;
	$note_prix_soalazaina = $data['note_prix_soalazaina'];
	$difference_prix_soalazaina = $pu_soalazaina - $prix_avant_soalazaina;
	
	if ($difference_prix_soalazaina != 0) {
		$note_prix_soalazaina = "NEW";
		$change_prix = "YES";
		//history
		$before_change = $before_change.' | PU Soalazaina : '.$prix_avant_soalazaina.'</br>';
		$after_change = $after_change." | PU Soalazaina : <span class ='text-danger'><b>".$pu_soalazaina.'</b></span></br>';
		//To handle empty note field
		if (strlen($note_x) == 0) {
			$note_x = "latest";
		}
	}else{
		$difference_prix_soalazaina = $data['difference_prix_soalazaina'];
	}
	//echo $difference_prix_soalazaina;
	...
	if ($difference_prix_amparafa != 0) {
		...
		$change_prix = "YES";
		//history
		$before_change = $before_change.' | PU Afa : '.$prix_avant_amparafa.'</br>';
		$after_change = $after_change." | PU Afa : <span class ='text-danger'><b>".$pu_aparafa.'</b></span></br>';
		//To handle empty note field
		if (strlen($note_x) == 0) {
			$note_x = "latest";
		}
	...
	//-------------------------------------
//-------------------------------------------
	//echo $pu_soalazaina.$note_prix_soalazaina.$difference_prix_soalazaina;
	//STOP QUERY if no change in prix field
	if ($change_prix == "YES") {
		
		$query_c = "UPDATE produit
		            SET prix_de_vente = ?,pu_aparafa = ?,pu_ambato_tantely = ?,pu_soalazaina = ?,prix_fournisseur = ?, benefice = ?, note_x = ?, note_prix = ?, note_prix_tantely = ?,note_prix_soalazaina = ?, note_prix_amparafa = ?,difference_prix = ?, difference_prix_tantely = ?,difference_prix_soalazaina = ?, difference_prix_amparafa = ?
		            WHERE id_x = ?";

			$q = $bdd->prepare($query_c);

			$q->execute(array($prix_de_vente,$pu_aparafa,$pu_tantely,$pu_soalazaina, $prix_fournisseur, $benefice,$note_x, $note_prix,$note_prix_tantely,$note_prix_soalazaina,$note_prix_amparafa, $difference_prix_global,$difference_prix_tantely,$difference_prix_soalazaina,$difference_prix_amparafa, $id_x));	
