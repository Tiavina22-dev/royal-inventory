...
//----------GET DATA IN USER TABLE---------------
       ...
       while ($donnees = $reponse->fetch())
         ...
         $numero_stock = $donnees['stock_numero_commande'];
         $nom_du_client = $donnees['stock_client_name'];
...
$query_c = "INSERT INTO stock_prep(numero_stock_prep, nom_du_client,description_date, id_x, qt, prix_de_vente, state, note,user_stock_prep) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	$q->execute(array($numero_stock, $nom_du_client,$description_date, $id_x, $qt, $prix_fournisseur, $state, $note_stock, $username));
  $q->closeCursor();
//-------Actual_id_x---------------------
$reponse = $bdd->prepare('SELECT * FROM stock_prep');
$reponse->execute(array());
