...
$reponse = $bdd->prepare("SELECT MAX(numero_commande_stock) as numero FROM mvt WHERE type_de_mvt = 'vente';");
        $reponse->execute(array());
...
$reponse = $bdd->prepare("SELECT MAX(numero_commande) as numero FROM commande;");
        $reponse->execute(array());
...
$q->closeCursor();
//-------------UPDATE USER MEMO-------------
$query_c = 'UPDATE user SET point_de_vente = ?, numero_commande = ?, description_date = ? WHERE User_Name = ?';
	$q = $bdd->prepare($query_c);
