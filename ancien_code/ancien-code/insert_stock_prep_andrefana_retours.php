...
$reponse = $bdd->prepare('SELECT * FROM stock_prep_calc');
...
//-------------------------------------------
    ...
    //echo ('$numero_stock : '.$numero_stock.'</br> $nom_du_client : '.$nom_du_client.'</br> $description_date : '.$description_date.'</br> $id_x : '.$id_x.'</br> $qt : '.$qt.'</br> $prix_de_vente : '.$prix_de_vente.'</br> $state : '.$state.'</br> $note_stock : '.$note_stock.'</br> $username : '.$username);
$query_c = "INSERT INTO stock_prep_calc(numero_stock_prep, nom_du_client,description_date, id_x, qt, prix_de_vente, state, note,user_stock_prep) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
