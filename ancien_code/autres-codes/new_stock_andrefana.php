...
$reponse = $bdd->prepare("SELECT MAX(numero_commande_stock) as numero FROM mvt_calc WHERE type_de_mvt = 'stock';");
        $reponse->execute(array());
...
$reponse = $bdd->prepare("SELECT MAX(numero_commande) as numero FROM stock_prep_calc;");
        $reponse->execute(array());
...
if (isset($_POST['description_date']))
{
	...
	$description_date = "Inventaire/Ajout du ".$daty.$Suffix;
	}
}
//-------------UPDATE MEMO-------------
