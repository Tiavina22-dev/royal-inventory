...
<br>
<br>
<h4 id="ancre1" class="text-center">LES VENTES AMBATO TANTELY</h4>
<?php
//Connect to BD
include('connect.php');

   //Query to liste searched product
   $query_stock_inventaire = "SELECT prix_unitaire,pu_ambato_tantely,type_de_mvt,produit.id_x as id_x, nom_x, pu_ambato_tantely, (SUM(qt)*(-1)) as sm, reference_x, note_x,((prix_unitaire-pu_ambato_tantely)*100/(pu_ambato_tantely)) as pourcentage,ref_commande_stock,(prix_unitaire-pu_ambato_tantely) as tombony  FROM mvt NATURAL JOIN produit WHERE nom_client_fournisseur ='Ambato_Tantely' AND type_de_mvt='vente' GROUP BY id_x ORDER BY tombony DESC;";
...
<!------------------------------------------------->
        ...
        <td><?php echo $donnees['pu_ambato_tantely']; ?></td>
