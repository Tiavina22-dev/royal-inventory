...
<a href="vente_recap_legere_ambato_1.php"><button type="submit" class="btn btn-info">RETOURS</button></a>
</div>
<br>
<br>
<br>
...
//Query to get path/Date/Note
   $query_path_date = 'SELECT * from recap_vente WHERE no_activite = ?';
	...
	while ($donnees = $query_path_date -> fetch())
	{
		...
		$benefice_aparafa = $donnees['difference_aparafa'];
		$resolution = $donnees['resolution'];
		$mihoatra = $donnees['mihoatra']+0;
		$royal_versement = $donnees['Montant'];		
	}
?>
<div>
   ...
   <table class="table-info table">
        <thead>
        	<tr>
        		...
        		<th rowspan="2">QT</th>
        		<th colspan="2" class="text-secondary text-center">ROYAL</th>
        		<th colspan="3" class="text-danger text-center">AMBATO</th>
        		<th rowspan="2">NOTE</th>
        		<th rowspan="2">By</th>
        		<th colspan="2" rowspan="2">Modifier/suprimer</th>
        	</tr>
        <tr>
        <th class="text-secondary text-right">PU</th>
        <th class="text-secondary text-right">MT</th>
        <th class="text-danger text-right">PU</th>
        <th class="text-danger text-right">MT</th>
        <th class="text-danger text-right">BNC</th>
        <?php
        if ($nom_client_fournisseur == 'Amparafa') {
...
$sm_mt_afa = ($donnees['prix_aparafa']*$donnees['qt']*(-1))+$sm_mt_afa;
...
<!------------------------------------------------->
        ...
        <td class="text-secondary text-right"><?php echo $donnees['prix_unitaire']; ?></td>
        <td class="text-secondary text-right"><b><?php echo ($donnees['prix_unitaire']*abs($donnees['qt'])); ?></b></td>
        <td class="text-danger text-right"><?php echo $donnees['prix_aparafa']; ?></td>
       	<td class="text-danger text-right"><?php echo ($donnees['prix_aparafa']*abs($donnees['qt'])); ?></td>
       	<td class="text-danger text-right"><b><?php echo $benefice_aparafa; ?></b></td>
        <?php
        if ($nom_client_fournisseur=="Amparafa") {
       	?>
       	<?php
		...
		<td><a class="center" href="delete_mvt_vente.php?id_mvt=<?php echo $donnees['id_mvt']; ?>&description_date=<?php echo $description_date; ?>&nom_client_fournisseur=<?php echo $nom_client_fournisseur; ?>" onclick="confirmationDelete('Do you want to DELETE this line?');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit" /></a></td>
		---------->
		<td><a class="center" href="#" onclick="confirmationDelete('INTERDIT de Supprimer une vente | Votre action sera enregister');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit" /></a></td>
		...
		<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="<?php echo "no".$donnees['id_mvt']; ?>" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<div class="modal-body">
					<!-- actual form -->
					<form role="form" action="modifier_mvt_vente_ambato_1.php" method="post">
						<div class="form-group">
						...
						<div class="form-group">
						<label>PU ROYAL</label>
						<input class="form-control btn-secondary" name="prix_unitaire" value="<?php echo $donnees['prix_unitaire']; ?>" type="number" step="any">
						...
						<div class="form-group">
						<label>PU AMAROTANA</label>
						<input class="form-control btn-danger" name="prix_aparafa" value="<?php echo $donnees['prix_aparafa']; ?>" type="number" id="<?php echo $textzone_id2; ?>">
						</div>
						<div class="form-group">
...
?>
	<tr>
        ...
        <th></th>
        <th></th>
        <th class="text-secondary text-right"></th>
        <th class="text-secondary text-right"><?php echo $sm_mt;?></th>
        <th class="text-danger text-right"></th>
        <th class="text-danger text-right"><?php echo $sm_mt_afa;?></th>
        <th class="text-danger text-right"><?php echo $bnc_total;?></th>
        <?php
        if ($nom_client_fournisseur == 'Amparafa') {
        }
        ?>
