...
<br>
<div class="text-center">
<form role="form" action="missing_product_cookies.php" method="post">
  <div>
    ...
    <option value="none" selected disabled hidden>Choose Point de vente</option>
      ...
      <option value="MoraranoCh" <?php if($selected == '6'){echo("selected");}?>>MoraranoCh</option>
      ...
      <option value="Soalazaina" <?php if($selected == '8'){echo("selected");}?>>Ranto Soalazaina</option>
       <option value="Ambato_Pneu" <?php if($selected == '8'){echo("selected");}?>>Ambato_Pneu</option>
      <!--
...
<?php
    ...
    $num_stock = "";
    $description_date = "";
		$query_client_name = $bdd->prepare("SELECT * FROM stock_prep WHERE description_date LIKE '%Rectifier%' AND nom_du_client = ?");
        $query_client_name->execute(array($nom_client_fournisseur));
...
?>
	...
	<div>
		<input id="ancre1" type="search" class="btn light-table-filter" data-table="table-bordered" placeholder="Filter/Search">
...
//------------------------------------------------------
  ...
  if (($qte > 0)) {
    ...
    if ($hidde == "NO") {
      $no = $no + 1;
		//Handle table color
      if ($color_line == 'bg-light') {
      	$color_line = "bg-warning";
      } else {
      	$color_line = 'bg-light';
      }
      
?>
	<tr class=<?php echo $color_line; ?>>
			
		<td><a href="#ancre1"><?php echo $no; ?></a></td>
		<td><?php echo $donnees['reference_x']; ?></td>
		<td class="text-left"><?php echo $donnees['nom_x']; ?></td>
