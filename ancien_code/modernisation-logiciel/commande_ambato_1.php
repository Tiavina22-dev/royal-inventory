...
<h5 class="text-center"><b><span class="text-success"><?php echo $msg_validation; ?></span><a class="center" href="anuler_validation_ambato_1.php?numero_commande=<?php echo $numero_commande; ?>" onclick="confirmationDelete('Anuler la validation?');return false; post ;"><span class="text-danger"><?php echo $annuler; ?></span></a></b></h5>
<h5 class="text-center text-info"><a class="center" href="vente_recap_legere_ambato_1.php"><b><?php echo $controle; ?></b></a></h5>
<?php
#####################MULTI-SESSION######################
...
<a href="stock_reduit.php" target="_blank" rel="noopener noreferrer"><button type="button" class="btn btn-success">Stock Retirer / Retours</button></a>
</div>
<div class="text-center">
...
if (isset($_COOKIE['key_word'])) 
{
   ...
   //VERIFY CHECKBOX
	...
	if ($checkbox == 'TRUE') {
		...
		$query_product_search = "SELECT produit.id_x as id_x, nom_x, prix_de_vente,prix_fournisseur, qt, reference_x, note_x,img_path_x,pu_aparafa,pu_ambato_tantely FROM mvt INNER JOIN produit ON mvt.id_x = produit.id_x WHERE (nom_x LIKE ? OR reference_x LIKE ? OR produit.id_x LIKE ?) AND type_de_mvt='stock' AND nom_client_fournisseur = ? GROUP BY reference_x ORDER BY date_time";
		$q = $bdd->prepare($query_product_search);

		$q->execute(array("%".$key_word."%", "%".$key_word."%", "%".$key_word."%" , $client_name));
	} else {
		# GLOBAL PRODUIT
		$query_product_search = 'SELECT produit.id_x as id_x, nom_x, prix_de_vente,prix_fournisseur, qt, reference_x, note_x,img_path_x,pu_aparafa,pu_ambato_tantely FROM mvt RIGHT JOIN produit ON mvt.id_x = produit.id_x WHERE nom_x LIKE ? OR reference_x LIKE ? OR produit.id_x LIKE ? GROUP BY reference_x ORDER BY date_time';
		$q = $bdd->prepare($query_product_search);
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<form role="form" action="insert_commande_ambato_1.php" method="post">
						...
						<input class="form-control btn btn-warning" name="prix_ambato" value="<?php echo $donnees['pu_ambato_tantely']+0; ?>" type="number" id="<?php echo $textzone_id5; ?>" step="any" oninput="Montant3Function($(this));">
						</div>
						<div class="form-group">
						...
						<?php if ($client_name == 'Amparafa') {}else{; ?>
						<div>
						<input class="form-control btn btn-danger" id="<?php echo $textzone_id; ?>" style="display:none">
						</div>
						<?php }; ?>
						<div class="form-group">
...
<!-------------------------------------->
	...
	<?php if ($nb_waiting != 0) {
	?>
	<a href="valider_waiting_commande_ambato_1.php" onclick="confirmationDelete('Valider le commande?');return false; post ;"><button class="btn btn-danger" id="Verroullier1">Verroullier (Payment Effectué)</button></a>
	<?php
		}
	?>
	<a href="commande_ambato_1_export.php"><button class="btn btn-warning">PDF ROYAL</button></a>
	</div>
	<div class="form-group text-left container">
	<button class="btn btn btn-light" disabled><span id="selected"><img src="img/loading6.gif" height="30" width="30" background alt="Edit"/></span> <b>LIGNE SELECTIONNE</b></button>
	<button class="float-right btn btn-light" disabled><b>NET A PAYER</b> <span id="total_waiting2"><img src="img/loading6.gif" height="30" width="30" background alt="Edit"/></span></button>
	</div>
	 <table class="table-info table container">
        <thead class="table-dark">
        <tr>
        	<th class="text-center" colspan="13"><?php echo $description_date." (Activiter No. ".$num_stock.")"; ?></th>
        </tr>
        <tr>
        <th></th>
        <th>No.</th>
        ...
        <th class="text-right">PU AMBATO</th>
        <th class="text-right">MT AMBATO</th>
        <th>BENEFICE</th>
        <th>Remarque</th>
        <th>Verrou</th>
...
?>
		...
		<td class="<?php echo $color_waiting; ?>">
			...
			<input type="checkbox" value=<?php echo ($donnees['prix_aparafa']*$donnees['qt']); ?> name="groupChecked" id="<?php echo $donnees['id_commande']; ?>" onclick="getcheckboxvalue()" checked>
			<?php } ?>
		</td>
		...
		<td class="text-right <?php echo $color_waiting; ?>"><?php echo number_format(($donnees['prix_aparafa']*$donnees['qt']),0, "", " "); ?></td>
		<td class="text-right"><span class="w3-badge <?php echo $color_badge; ?>"><?php echo $aff_difference_prix; ?></span></td>
		<td class="<?php echo $color_waiting; ?>"><?php echo $product_note; ?></td>
		<td>
			<?php
        	if ($donnees['state']=='preparing') {
        	?>
        	<a style="<?php echo $state_suppr; ?>;" class="center" title='VERROULLIER' href="lock_commande_ambato_1.php?id_commande=<?php echo $donnees['id_commande']; ?>" onclick="confirmationDelete('Do you want to lock this line?');return false; post ;"><img src="img/lock.png" height="30" width="30" background alt="Edit"/></a>
        	<?php
        	} else {
        	?>
        	<a id="<?php echo $ib.'L'; ?>" style="<?php echo $state_suppr; ?>;" class="center" href="unlock_commande_ambato_1.php?id_commande=<?php echo $donnees['id_commande']; ?>" onclick="confirmationDelete('Do you want to unlock this line?');return false; post ;"><img src="img/unlock.png" height="30" width="30" background alt="Edit"/></a>
        	<?php
        	}
        	?>
		</td>
		...
		<td>
        	...
        	if ($donnees['state']=='preparing') {
        	?>
			<a style="<?php echo $state_suppr; ?>;" title='SUPPRIMER' class="center" href="delete_commande_ambato_1.php?id_commande=<?php echo $donnees['id_commande']; ?>" onclick="confirmationDelete('Do you want to DELETE this line?');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit"/></a>
			<?php
        	} else {
        	?>
			<a id="<?php echo $ib; ?>" style="<?php echo $state_suppr; ?>;" class="center" href="delete_commande_ambato_1.php?id_commande=<?php echo $donnees['id_commande']; ?>" onclick="confirmationDelete('Do you want to DELETE this line?');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit"/></a>
			<?php
        	}
        	?>
		</td>
			...
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<form role="form" action="modifier_commande_ambato_1.php" method="post">
						...
						<input class="form-control btn btn-light" name="prix_de_vente" value="<?php echo $donnees['prix_unitaire']; ?>" type="number" oninput="MontantFunctionModif($(this));" id="<?php echo $id_pu_modif; ?>">
						</div>
						<div class="form-group">
						...
						<input class="form-control btn btn-warning" name="prix_fournisseur" value='<?php echo ($donnees['prix_aparafa']+0);?>' type="number" step = "any" id="<?php echo $textzone_id2; ?>" oninput="Montant3FunctionModif($(this));">
						</div>
						<div class="form-group">
...
?>
		        ...
		        <th class="text-right">GLOBAL</br><span class="text-secondary">ROYAL</span></br><span class="text-primary">BENEFICE</span></th>
		        <th  class="text-right"><?php echo number_format($total_commande_ambato,0, "", " "); ?></br><span class="text-secondary"><?php echo number_format($total_commande,0, "", " "); ?></span></br><span class="text-primary"><?php echo number_format($difference_prix_total,0, "", " "); ?></span></th>
		        <th class="text-danger">A PAYER <a href="" data-toggle="modal" data-target="#famerimbola_click"><span id="total_waiting"><?php if ($total_waiting != 0) {echo number_format(($total_waiting),0, "", " ");}  ?></span></a></th>
		        <th class="text-primary text-right"><?php echo number_format($difference_prix_total,0, "", " "); ?></th>
		        <th colspan="4" class="text-danger"></th>
<!-------FORM FAMERIMBOLA-------->
		...
		<div aria-hidden="true" aria-labelledby="myModalLabel" role="dialog" tabindex="-1" id="famerimbola_click" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					<div class="modal-header">
					<h4 class="modal-title">AUTO CALCULATION</h4>
					<button aria-hidden="true" data-dismiss="modal" class="close" type="button">&times</button>
					</div>
					<div class="modal-body">
					<!-- actual form -->
					<form role="form" action="" method="post">
						<div class="form-group">
						<label>SOMME RECU</label>
						<input class="form-control btn btn-warning" name="prix_de_vente" value="<?php echo $total_waiting; ?>" type="number" oninput="FamerimbolaFunction($(this));" id="vola_voaray">
						</div>
						<div class="form-group">
						<label>SOMME A PAYER</label>
						<input class="form-control btn btn-success" name="qt" value="<?php echo number_format($total_waiting,0, "", " "); ?>" type="text" id="net_a_payer" disabled>
						</div>
						<div class="form-group">
						<label class="text-danger"><b>A RENDRE</b></label><span class="w3-badge w3-right w3-margin-right w3-red" id="famerimbola">0</span>
						</div>
					</form>
					<!-- actual form ends -->
					</div>
...
{
	?>
<a href="valider_waiting_commande_ambato_1.php" onclick="confirmationDelete('Valider le commande?');return false; post ;"><button class="btn btn-danger" id="Verroullier2">Verroullier (Payment Effectué)</button></a>
...
?>
<a href="commande_ambato_1_export.php"><button class="btn btn-warning">PDF ROYAL</button></a>
<input name="vente_id_array" class="btn btn-light float-right" type="password" oninput="unlock($(this));" placeholder="Mot de Passe">
</div>
<br>
<!--
...
<div class="container">
        ...
        <thead>
        <tr>
        	<th class="text-center bg-secondary text-light" colspan="6">DEPENSE</th>
        </tr>
        <tr>
            <th>No.</th>
	        <th>ID</th>
	        <th>MOTIF</th>
	        <th class="text-right">DEPENSE AMBATO</th>
...
?>
	<tr>
		...
		<td class="text-right"><?php echo number_format($donnees['depense_aparafa'],0, "", " "); ?></td>
		<td class="text-right"><?php echo number_format($donnees['montant'],0, "", " "); ?></td>
		<td><a class="center" href="delete_depense_ambato_1.php?id_depense=<?php echo $donnees['id_depense']; ?>" onclick="confirmationDelete('Do you want to DELETE this line?');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit" /></a></td>
	</tr>
<?php
}
$query_depense_list->closeCursor();
?>
		<tr>
	        ...
	        <th class="text-right"><?php echo number_format($total_depense_aparafa,0, "", " "); ?></th>
	        <th class="text-right"><?php echo number_format($total_depense,0, "", " "); ?></th>
		    <th class="text-right"><?php echo "GT: ".number_format(($total_depense+$total_depense_aparafa),0, "", " ");?></th>
        </tr>
</tbody>
</table>
</div>
...
<h5 class="text-center text-success"><?php echo $msg_ok; ?></h5>
	...
	<div class="container">
        ...
        <thead>
        <tr class="bg-primary">
        	<th class="text-center" colspan="9">RETOURS AUJOUR'HUI</th>
        </tr>
        <tr>
        <th>No.</th>
        <th>Ref</th>
...
?>
		...
		<td><a class="center" href="delete_stock_prep_reduit.php?id_stock_prep=<?php echo $donnees['id_stock_prep']; ?>" onclick="confirmationDelete('Do you want to DELETE this line?');return false; post ;"><img src="img/deleteicon2.png" height="30" width="30" background alt="Edit" /></a></td>
		<?php
		}//Close else 
		?>
<!-------FORM DE MODIFIER-------->
...
--->
<p class="form-control text-center text-light" style="background-color: purple"><b>ROYAL (TL:<?php echo floor($total_commande);?> Depense:<?php echo $total_depense;?> Reste:<?php echo floor($total_commande-$total_depense);?>) || AMBATO(TL:<?php echo floor($difference_prix_total);?> Depense:<?php echo floor($total_depense_aparafa);?> Reste:<?php echo floor($difference_prix_total-$total_depense_aparafa);?>) || RESTE GLOBAL = <?php echo floor($sous_total_aparafa-$total_depense_aparafa-$total_depense);
	    ?>
</b></p>
...
<input class="form-control bg-light" name="path" placeholder="Copie-coller l'emplacement du photo ici" type="text" hidden>
</div>
<div class="form-group text-center">
<a id="annuler_link" style="<?php echo $state_suppr; ?>;" href="anuler_commande_ambato_1.php" onclick="confirmationDelete('Anuler le commande?');return false; post ;"><button id="annuler_button" type="button" class="btn btn-danger" <?php echo $state_button; ?>>Anuler</button></a>
...
<form role="form" action="insert_depense_ambato_1.php" method="post">
	<div class="form-group">
		<label id="textzoned3"><b>DEPENSE AMBATO</b> (Ariary)</label>
		<input class="form-control btn btn-light" name="depense_aparafa" value=0 type="number" id="textzoned2">
	</div>
	<div class="bg-warning">
		<input type="checkbox" id="myCheckd" onclick="dFunction()">
		<label><b>DEPENSE ROYAL</b></label>
		<input class="form-control btn btn-danger" name="depense" value=0 type="number" id="textzoned" style="display:none">
	</div>
	<div class="form-group">
		<label>Motif</label>
