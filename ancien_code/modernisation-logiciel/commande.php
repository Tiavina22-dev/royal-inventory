...
<div>
        ...
        <tr>
	        ...
	        <th>ID</th>
	        <th>MOTIF</th>
	        <th class="text-right">DEPENSE MAHAVANTANA</th>
	        <th></th>
	        <th>Suppr</th>
        </tr>
        </thead>
...
<p class="form-control text-center text-light container" style="background-color: purple"><b>MT MAHAVANTANA =<?php echo floor($total_commande);?> | Depense MAHAVANTANA:<?php echo $total_depense;?> <?php
	    if ($client_name=='Amparafa' OR $client_name=='Bejofo' OR $client_name=='Soalazaina') {
...
if ($decision_new_commande == 0)
{
   ...
   $query = "SELECT description_date from mvt WHERE type_de_mvt = 'vente' AND nom_client_fournisseur ='Ambato_Tantely' GROUP BY description_date ORDER BY id_mvt DESC;";
	...
	//---------------------------------------
   	//################AMPARAFA#####################
		   $query = "SELECT description_date from mvt WHERE type_de_mvt = 'vente' AND nom_client_fournisseur ='Ambato_Pneu' GROUP BY description_date ORDER BY id_mvt DESC;";
    $qc = $bdd->prepare($query);
    ...
    $chaine = "";
    $date1 = "";
    $latest_date_v_miangaly = "09/09/20";
    $liste_v_miangaly = "";
    //$donnees = $qc -> fetch();
    //$latest_date = $donnees['description_date'];
    while ($donnees = $qc -> fetch())
    {
        $chaine =  $donnees['description_date'].' ';
        $liste_v_miangaly = $liste_v_miangaly.'# '.$donnees['description_date'].'<br>';
        $x = chr(35).'/0-9-';
        preg_match("'([0-9]{2,4}[$x]{1,2}[0-9]{2,4}[$x]{1,2}[0-9]{2,4})'", $chaine, $res_regex);
        //Prise en compte separation date / or -
        if (isset($res_regex[1])) {
        ...
        //Eviter wrong format and unwanted space for $latest_date
        $latest_date_v_miangaly = str_replace('- ', '/', $latest_date_v_miangaly);
        $latest_date_v_miangaly = str_replace('/ ', '/', $latest_date_v_miangaly);
        $latest_date_v_miangaly = str_replace(' / ', '/', $latest_date_v_miangaly);
        $latest_date_v_miangaly = str_replace(' -', '/', $latest_date_v_miangaly);
        $latest_date_v_miangaly = str_replace(' - ', '/', $latest_date_v_miangaly);
		...
		$latest_en = DateTime::createFromFormat('d/m/Y', $latest_date_v_miangaly);
		$latest_en = $latest_en -> format('y/m/d');
        if ($latest_en < $date1_en) {
            $latest_date_v_miangaly = $date1 ;
            //echo '#'.$latest_date.'<br>';
        }
		...
		$latest_date_v_miangaly_c = $latest_date_v_miangaly;
		//Convert date to format YYYY/mm/dd
        $latest_date_v_miangaly = DateTime::createFromFormat('d/m/y', $latest_date_v_miangaly);
		$latest_date_v_miangaly = $latest_date_v_miangaly -> format('Y-m-d');
		//------next day-------SOLUTION 1-----------------
		//$latest_date_v_tantely = explode('/', $latest_date_v_tantely);
		//$latest_date_v_tantely = Date("Y/m/d",mktime(0,0,0,$latest_date_v_tantely[1],$latest_date_v_tantely[2]+1,$latest_date_v_tantely[0]));
		//------next day-------SOLUTION 2-----------------
		$latest_date_v_miangaly = Date("Y-m-d",strtotime('+1 day',strtotime($latest_date_v_miangaly)));
		//------------------------------------
   		$qc->closeCursor();
   		//Get day name
   		$day_miangaly = Date('l', strtotime($latest_date_v_miangaly));
   		if ($day_miangaly == 'Sunday') {
   			$latest_date_v_miangaly = Date("Y-m-d",strtotime('+1 day',strtotime($latest_date_v_miangaly)));
   			$day_miangaly = Date('l', strtotime($latest_date_v_miangaly));
   		}
   		//-----------------------------------------
		$latest_date_v_miangaly_a = DateTime::createFromFormat('Y-m-d', $latest_date_v_miangaly);
		$latest_date_v_miangaly_a = $latest_date_v_miangaly_a -> format('d/m/y');
		$qc->closeCursor();
   //Query to get date of Journal de vente Ambato_Tantely
   ...
   //################VEVE#####################
   //Query to get date of Journal de vente Ambato_Tantely
   $query = "SELECT description_date  from mvt WHERE type_de_mvt = 'vente' AND nom_client_fournisseur ='MoraranoCh' GROUP BY description_date ORDER BY id_mvt DESC;";
    $qc = $bdd->prepare($query);
    ...
    while ($donnees = $qc -> fetch())
    {
        $liste_v_MoraranoCh = $liste_v_MoraranoCh.'# '.$donnees['description_date'].'<br>';
        $chaine =  $donnees['description_date'].' ';
        $x = chr(35).'/0-9-';
        preg_match("'([0-9]{2,4}[$x]{1,2}[0-9]{2,4}[$x]{1,2}[0-9]{2,4})'", $chaine, $res_regex);
        //Prise en compte separation date / or -
        ...
        //echo '>>'.$date1.'<br>';
		    ...
		    //Convert date to format YYYY/mm/dd
		$latest_date_v_MoraranoCh_c = $latest_date_v_MoraranoCh;
        $latest_date_v_MoraranoCh = DateTime::createFromFormat('d/m/y', $latest_date_v_MoraranoCh);
		$latest_date_v_MoraranoCh = $latest_date_v_MoraranoCh -> format('Y-m-d');
		//------next day-------SOLUTION 2-----------------
		$latest_date_v_MoraranoCh = Date("Y-m-d",strtotime('+1 day',strtotime($latest_date_v_MoraranoCh)));
		//------------------------------------
   		$qc->closeCursor();
   		//Get day name
   		$day_MoraranoCh = Date('l', strtotime($latest_date_v_MoraranoCh));
   		if ($day_MoraranoCh == 'Sunday') {
   			$latest_date_v_MoraranoCh = Date("Y-m-d",strtotime('+1 day',strtotime($latest_date_v_MoraranoCh)));
   			$day_MoraranoCh = Date('l', strtotime($latest_date_v_MoraranoCh));
   		}
   		//-----------------------------------------
		$latest_date_v_MoraranoCh_a = DateTime::createFromFormat('Y-m-d', $latest_date_v_MoraranoCh);
		$latest_date_v_MoraranoCh_a = $latest_date_v_MoraranoCh_a -> format('d/m/y');
   $qc->closeCursor();
   //################AMBAIBOHO TOLE#####################
   //Query to get date of Journal de vente Ambato_Tantely
...
{ 
   ...
   //------------------------------------
	...
	//--- 1. FANADIOVANA NY DATY "Journal du 04/03/26" ---
$date_compare = date('Y-m-d'); 
if (!empty($description_date)) {
    // Esorina ny "Journal du " ary diovina ny habaka
    $clean_date = trim(str_replace('Journal du ', '', $description_date));
    
    // Avadika ho format SQL (YYYY-MM-DD)
    $d_obj = DateTime::createFromFormat('d/m/y', $clean_date);
    if (!$d_obj) { $d_obj = DateTime::createFromFormat('d/m/Y', $clean_date); }
    
    if ($d_obj) {
        $date_compare = $d_obj->format('Y-m-d');
    }
}

//--- 2. QUERY SQL: Maka ny PU (prix_unitaire) farany talohan'ny daty voafidy ---
$query_pu_Mvt = "SELECT prix_unitaire FROM mvt 
                 WHERE type_de_mvt = 'stock' 
                 AND id_x = ? 
                 AND nom_client_fournisseur = ? 
                 AND Date_du_Journal_mvt <= ? 
                 ORDER BY Date_du_Journal_mvt DESC, id_mvt DESC LIMIT 1";

$qpc = $bdd->prepare($query_pu_Mvt);
$qpc->execute(array($donnees['id_x'], $client_name, $date_compare));
$res_mvt = $qpc->fetch();

//--- 3. NY VIDINY RAISINA (Fallback) ---
// Raha nahita PU tao amin'ny mvt izy dia iny no raisina, raha tsy izany dia ny $pu standard
$prix_final = ($res_mvt && $res_mvt['prix_unitaire'] > 0) ? $res_mvt['prix_unitaire'] : $pu;
			...
			<div class="modal-dialog">
				<div class="modal-content btn-brown">
					...
					<form role="form" action="insert_commande.php" method="post">
						...
						<input class="form-control text-center" style="background: #d4e157" name="qt" value="1" type="number" step="any" id="<?php echo $id_qt; ?>" oninput="Montant2Function($(this));">
						</div>
						<div class="form-group">
					    <label>PRIX ANOMEZANA MAHAVANTANA (Ariary)</label>
					    <!-- Badge mampiseho ny PU farany hita -->
					    <span class="w3-badge w3-right w3-margin-right w3-green" id="<?php echo $id_montant; ?>">
					        <?php echo number_format($prix_final, 0, "", " "); ?>
					    </span>
					    
					    <!-- Input automatique misy ilay PU voadio -->
					    <input class="form-control btn btn-danger" name="prix_de_vente" 
					           value="<?php echo $prix_final; ?>" 
					           type="number" step="any"
					           oninput="MontantFunction($(this));" 
					           id="<?php echo $id_pu; ?>">

					    <!-- Textzone hidden ho an'ny JS -->
					    <input class="form-control btn-danger" id="<?php echo $textzone_id; ?>" 
					           value="<?php echo $prix_final; ?>" style="display:none">
						</div>
...
?>
	...
	<input type="button" class="float-right btn btn-secondary font-weight-bold text-light" value="<?php echo ($total_line); ?> Lignes">
	 ...
	 <table class="table-striped table container">
        ...
        <th class="text-right">MT MAHAVANTANA</th>
        <th>Note</th>
        <th>Modifier</th>
        <th>Supprimer</th>
        </tr>
			...
			<div class="modal-dialog">
				<div class="modal-content btn-brown">
					...
					<form role="form" action="modifier_commande.php" method="post">
						...
						<div class="form-group">
						<label>PRIX ANOMEZANA MAHAVANTANA(Ariary)</label><span class="w3-badge w3-right w3-margin-right w3-green" id="<?php echo $id_montant_modif; ?>"><?php echo $donnees['prix_unitaire']*$donnees['qt']; ?></span>
...
<div>
    ...
    <thead class="text-light">
        <tr>
            <th class="text-center bg-secondary text-light" colspan="8">
                DEPENSE <?php echo (strtoupper($client_name)." || ".$description_date." (Activiter No. ".$num_stock.")"); ?>
            </th>
        </tr>
        <tr>
            <th>No.</th>
    ...
    <tbody class="text-light">
        ...
        <tr>
            ...
            <td class="text-center">
                <!-- Bouton Modifier -->
                <button class="button" data-toggle="modal" data-target="#editModal<?php echo $donnees['id_depense']; ?>">
                    <img src="img/edit_icon.png" height="30" width="30" background alt="Edit" />
                </button>
            </td>
            <td>
                <a href="delete_depense.php?id_depense=<?php echo $donnees['id_depense']; ?>" 
                   onclick="return confirmationDelete('Do you want to DELETE this line?');">
                   <img src="img/deleteicon.png" height="" width="30" alt="Delete" />
                </a>
            </td>
        </tr>

        <!-- Modal tsirairay -->
        <div class="modal fade" id="editModal<?php echo $donnees['id_depense']; ?>" tabindex="-1">
          <div class="modal-dialog">
            <div class="modal-content">
              <form method="post" action="modifier_depensec.php">
                <div class="modal-header bg-warning">
                  <h5 class="modal-title">Modifier Depense</h5>
                  <button type="button" class="close" data-dismiss="modal">&times;</button>
                </div>
                <div class="modal-body">
                  <input type="hidden" name="id_depense" value="<?php echo $donnees['id_depense']; ?>">
                  <div class="form-group">
                    <label>Motif</label>
                    <input type="text" class="form-control" name="motif" 
                           value="<?php echo $donnees['motif']; ?>" required>
                  </div>
                  <div class="form-group">
                    <label>Montant</label>
                    <input type="number" class="form-control" name="montant" 
                           value="<?php echo $donnees['montant']; ?>" required>
                  </div>
                </div>
...
<p class="form-control text-center text-light" style="background-color: purple"><b>MT MAHAVANTANA =<?php echo Round($total_commande);?> | Depense MAHAVANTANA: <?php echo $total_depense;?> | Reste MAHAVANTANA=<?php echo floor($total_commande-$total_depense);?>
	    <?php
...
<div>
	...
	<label class="float-right"><b>FANAMARIHANA</b>##########</label>
</div>
<div>
...
<select class="float-right" style="background: red" name="point">
		<option value="congratulation">Nilamina</option>
	    <option value="avertissement">be de be ny tsy mety</option
	     value="avertissement">vitsivittsy ny tsy mety</option>
</select>
</div>
<div>
<br>
...
<script>
	...
	if (shop == 'Ambato_Pneu') {
		var next_day = <?php echo json_encode($latest_date_v_miangaly); ?>;
		var day = <?php echo json_encode($day_miangaly); ?>;
	}
	if (shop == 'Amparafa') {
	...
	if (shop == 'MoraranoCh') {
		var next_day = <?php echo json_encode($latest_date_v_MoraranoCh); ?>;
		var day = <?php echo json_encode($day_MoraranoCh); ?>;
	}
...
<script>
	function Check_shop(e) {
		...
		if (shop == 'Ambato_Pneu') {
			var next_day = <?php echo json_encode($latest_date_v_miangaly_a); ?>;
			var current_day = <?php echo json_encode($latest_date_v_miangaly_c); ?>;
			var day = <?php echo json_encode($day_miangaly); ?>;
			//------DAY TO MALAGASY--------------
			if (day == 'Monday') {day = 'Latsinainy';}
			if (day == 'Tuesday') {day = 'Talata';}
		...
		if (shop == 'MoraranoCh') {
			var next_day = <?php echo json_encode($latest_date_v_MoraranoCh_a); ?>;
			var current_day = <?php echo json_encode($latest_date_v_MoraranoCh_c); ?>;
			var day = <?php echo json_encode($day_MoraranoCh); ?>;
			//------DAY TO MALAGASY--------------
			if (day == 'Monday') {day = 'Latsinainy';}
			if (day == 'Tuesday') {day = 'Talata';}
