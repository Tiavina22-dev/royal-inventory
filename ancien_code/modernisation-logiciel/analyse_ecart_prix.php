...
if (isset($_COOKIE['point_de_vente'])){
   ...
   //$query_stock_inventaire = "SELECT * FROM mvt NATURAL JOIN produit WHERE nom_client_fournisseur = ? AND type_de_mvt ='vente' AND Date_du_Journal_mvt >= ? GROUP BY id_x";
    $query_stock_inventaire = "SELECT * FROM (SELECT V.id_x,COUNT(V.id_x) FROM (SELECT * FROM mvt WHERE nom_client_fournisseur = ? AND type_de_mvt='vente' AND Date_du_Journal_mvt >= ? ORDER BY id_mvt) V INNER JOIN (SELECT *,MAX(value) as max FROM checking WHERE shop = ? AND checking_type = 'prix_different' GROUP BY id_x) C ON V.id_x = C.id_x WHERE numero_commande_stock > max GROUP BY V.id_x) Q1 NATURAL JOIN produit";
    $q = $bdd->prepare($query_stock_inventaire);

    $q->execute(array($point_de_vente, $starting_date ,$point_de_vente));
    //Number of Line
    $nb_line=$q->rowCount ();   
    if ($nb_line == 0) {
