...
else 
{
  ...
  $selected='0';
  if (isset($_COOKIE['point_de_vente_cookies'])) 
    {$nom_client_fournisseur = $_COOKIE['point_de_vente_cookies'];
...
<a class="center" href="controle_ambato.php"><button type="button" class="btn btn-warning">PRODUIT ABSENTS</button></a>
<br>
<br>
<div class="text-center">
<form role="form" action="new_product_cookies.php" method="post">
  <div>
    <select class="btn" name="point_de_vente">
    <option value="none" selected disabled hidden>Choose Point de vente</option>
      <option value="Ambatomainty" <?php  if($selected == '1'){echo("selected");}?>>Ambatomainty</option>
      ...
      <option value="Ambato_Pneu" <?php if($selected == '9'){echo("selected");}?>>Ambato_Pneu</option>
      <!--
...
<h1 class="text-danger">LES NOUVEAUX PRODUITS DANS LE NOUVEAU STOCK</h1>
</div>
<?php
...
{
   $query_ps = "SELECT produit.id_x,reference_x,nom_x,SUM(qt),pu_ambato_tantely FROM (SELECT stock_prep.id_x as id_x,nom_du_client,stock_prep.qt,stock_prep.description_date FROM mvt RIGHT JOIN (SELECT * FROM stock_prep WHERE nom_du_client = ? AND stock_prep.description_date LIKE '%Rectifier%') as stock_prep ON mvt.id_x = stock_prep.id_x WHERE stock_prep.nom_du_client = ? AND mvt.id_x IS NULL) as T1 INNER JOIN produit ON T1.id_x = produit.id_x GROUP BY produit.id_x ORDER BY nom_x;";
 }else{
//$limit = 200;
$query_ps = "SELECT produit.id_x,reference_x,nom_x,SUM(qt),pu_ambato_tantely FROM (SELECT stock_prep.id_x as id_x,nom_du_client,stock_prep.qt,stock_prep.description_date FROM mvt RIGHT JOIN (SELECT * FROM stock_prep WHERE nom_du_client = ? AND stock_prep.description_date LIKE '%Rectifier%') as stock_prep ON mvt.id_x = stock_prep.id_x WHERE stock_prep.nom_du_client = ? AND mvt.id_x IS NULL) as T1 INNER JOIN produit ON T1.id_x = produit.id_x GROUP BY produit.id_x ORDER BY nom_x;";
}
	...
	$query_stock_prep_list->execute(array($nom_client_fournisseur,$nom_client_fournisseur));	

 

$query_stock_prep = "SELECT produit.id_x,reference_x,nom_x,SUM(qt),pu_ambato_tantely FROM (SELECT stock_prep.id_x as id_x,nom_du_client,stock_prep.qt,stock_prep.description_date FROM mvt RIGHT JOIN (SELECT * FROM stock_prep WHERE nom_du_client = ? AND stock_prep.description_date LIKE '%Rectifier%') as stock_prep ON mvt.id_x = stock_prep.id_x WHERE stock_prep.nom_du_client = ? AND mvt.id_x IS NULL) as T1 INNER JOIN produit ON T1.id_x = produit.id_x GROUP BY produit.id_x ORDER BY nom_x;";
$query_stock_prep = $bdd->prepare($query_stock_prep);

$query_stock_prep->execute(array($nom_client_fournisseur,$nom_client_fournisseur));
//get row count
...
//if waiting commande exist set value of $waiting_commande to 1
 ...
 ?>
 <p>-------------AUCUN RESULTAT POUR <?php  echo $nom_client_fournisseur;?>--------------</p>
 <?php
 } else {
...
//--------GET LISTE OF STOCK AND VENTE---------------------
		//QUERY TO SHOW STOCK
	   $query_stock = "SELECT * FROM stock_prep WHERE id_x = ? AND stock_prep.description_date LIKE '%Rectifier%' AND nom_du_client = ? ";
	    $qs = $bdd->prepare($query_stock);

	    $qs->execute(array($donnees['id_x'],$nom_client_fournisseur));
	    $total_stock = 0;
	    $liste_stock = "";
     	$hors_cadre_stock = 0;
      	$hors_cadre_vente = 0;
      	$total_vente =0;
                        ...
                        { 
                          ...
                          //GET DATE ONLY
                          $liste_stock = $liste_stock.$donnees1['user_stock_prep'].'#<b>'.$donnees1['qt'].'</b>'.'<br>';
                        }
                        $qs->closeCursor();
	//PU for each point de vente
	//$pu = $donnees['prix_unitaire'];
  if ($nom_client_fournisseur == 'Ambato_Tantely') {
...
?>
	<tr>
		...
		<td class="text-right"><span class="w3-badge w3-right w3-margin-left; ?>"><b><?php echo ($total_stock-$hors_cadre_stock); ?></b></span></td>
